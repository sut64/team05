...
type RecieptHistory struct {
	gorm.Model
	RecieptCode  string    `gorm:"uniqueIndex" valid:"matches(^[R]\\d{4}$),required~RecieptCode: does not validate as matches(^[R]\\d{4}$)"`
	RecieptPrice float32   `valid:"positiveFloat~RecieptPrice must be equal or greater than 0"`
