...
func GetRepairHistory(c *gin.Context) {
	var repairhistory entity.RepairHistory
	id := c.Param("id")
	if err := entity.DB().Preload("RepairRequest").Preload("Difficulty").Preload("Editor").Raw("SELECT * FROM repair_histories WHERE id = ?", id).Find(&repairhistory).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func ListRepairHistories(c *gin.Context) {
	...
	if err := entity.DB().Preload("RepairRequest").Preload("Difficulty").Preload("Editor").Raw("SELECT * FROM repair_histories").Find(&repairhistories).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
